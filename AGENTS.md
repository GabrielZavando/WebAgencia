...
> **Monorepo:** único repo con contexto SDD compartido en la raíz.
> - `apps/api/` → API NestJS (`@webastro/api`)
> - `apps/web/` → Frontend Astro (`@webastro/web`)
> - Contexto agentico (fuente de verdad): `docs/`, `ai-specs/`, `openspec/`, `.opencode/` en la raíz.
> - No duplicar `docs/`, `ai-specs/`, `opencode.json`, `AGENTS.md` ni `openspec/` dentro de `apps/*`.
